...
export default async function handler(request: ServerlessRequest, response: ServerlessResponse) {
    ...
    if (contentLength > 20000) {
        return jsonResponse(response, { error: 'Payload too large' }, 413);
    }
    ...
    try {
        const body = (typeof request.body === 'string' ? JSON.parse(request.body) : request.body) as RegistrationPayload;
        const validationErrors = validatePayload(body);

        if (validationErrors.length > 0) {
            return jsonResponse(response, { error: 'Invalid registration payload', details: validationErrors }, 400);
        }
        ...
        return jsonResponse(response, responsePayload, webhookResponse.status);
    } catch {
        return jsonResponse(response, { error: 'Failed to process registration' }, 500);
    }
}
