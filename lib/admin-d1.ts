...
const resources = {
  ...
  karts_full: { columns: ['id', 'numero', 'modelo', 'categoria', 'motor', 'status', 'km_total', 'ultima_manutencao', 'proxima_manutencao', 'notes', 'ativo', 'created_at', 'chassi_numero', 'sensor_numero', 'redutor_antigo', 'redutor_novo', 'ultimo_piloto_equalizacao', 'traco_equalizacao', 'media_equalizacao_ms', 'melhor_equalizacao_ms', 'desvio_equalizacao_ms', 'ultima_equalizacao', 'manutencoes_pendentes'], search: ['numero', 'modelo', 'categoria', 'chassi_numero', 'sensor_numero'], defaultOrder: 'numero', readOnly: true },
...
async function syncClientesFromBridge(db: AdminD1Database, request: NextRequest, actor: string) {
  ...
  try {
    const response = await fetch(`${request.nextUrl.origin}/api/admin/clientes?limit=2000`, {
...
export async function handleAdminD1(request: NextRequest, path: string[], db: AdminD1Database, actor: string) {
  try {
    ...
    if (request.method === 'GET') {
      ...
      if (resourceName === 'clientes') await syncClientesFromBridge(db, request, actor);
