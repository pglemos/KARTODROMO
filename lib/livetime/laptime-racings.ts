...
function toRacing(row: RacingRow): LapTimeRacing {
  return {
    ...
    dataHora: toSqlDatetime(row.ExpectedDateTime)!,
    inicio: toSqlDatetime(row.StartDateTime),
...
function sqlConfig(options: LapTimeSqlOptions): sql.config {
  return {
    ...
    options: { encrypt: false, trustServerCertificate: true, ...(options.port ? {} : { instanceName: options.instanceName }) },
  };
}
...
function toPassing(row: PassingRow): LapTimePassing {
  return {
    id: String(row.Id_Passing),
    lapNumber: row.Lap,
    lapTime: formatSqlTime(row.LapTime),
    passTime: toSqlDatetime(row.TimeOfDay),
    speed: row.Speed,
    pos: row.Pos,
  };
}

export async function fetchLapTimePassings(
  options: LapTimeSqlOptions,
  racingCompetitorId: string,
  racingId: string,
): Promise<LapTimePassing[]> {
  const pool = new sql.ConnectionPool(sqlConfig(options));
  try {
    await pool.connect();
    const result = await pool
      .request()
      .input('competitorId', sql.BigInt, Number(racingCompetitorId))
      .input('racingId', sql.BigInt, Number(racingId))
      .query<PassingRow>(`
        select Id_Passing, Lap, LapTime, TimeOfDay, Speed, Pos
        from dbo.Passing
        where Id_RacingCompetitor = @competitorId and Id_Racing = @racingId
          and (DeletedLap = 0 or DeletedLap is null)
          and (InvalidLap = 0 or InvalidLap is null)
        order by Lap
      `);

    return result.recordset.map(toPassing);
  } finally {
    await pool.close().catch(() => undefined);
  }
}
