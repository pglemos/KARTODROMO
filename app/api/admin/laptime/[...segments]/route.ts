...
const ALLOWED_SLUGS = new Set(['bookings', 'booking-customers', 'racings', 'racing-competitors', 'passings']);
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ segments: string[] }> }) {
  ...
  // Try bridge first (dados reais do CRONO1 via tunnel/scraper)
  ...
  try {
    ...
    if (env.KARTODROMO_ADMIN_DB) {
      const d1Resource = slug === 'racings' ? 'corridas' : slug === 'racing-competitors' ? 'resultados' : slug === 'bookings' ? 'reservas' : null;
      if (d1Resource) {
