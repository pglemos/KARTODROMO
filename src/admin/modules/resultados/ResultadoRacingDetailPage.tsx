...
const Metric = ({ label, value, detail }: { label: string; value: string; detail?: string }) => (
  <Card className="p-4">
    <p className="text-[10px] font-bold uppercase tracking-[.14em] text-zinc-500">{label}</p>
      ...
      <thead className="border-b border-zinc-800 bg-zinc-950/60">
        <tr>
          {[
            ...
            <th className="whitespace-nowrap px-3 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500" key={label}>
              {label}
            </th>
          ))}
        </tr>
          ...
          <Card className="overflow-hidden">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-[900px] w-full text-left text-sm">
                <thead className="border-b border-zinc-800 bg-zinc-950/60"><tr>{['Kart', 'Piloto / equipe', 'Paradas válidas', 'Faltam', 'Erradas', 'Fora da janela', 'Punição'].map((label) => <th className="px-4 py-3 text-[10px] font-bold uppercase tracking-wider text-zinc-500" key={label}>{label}</th>)}</tr></thead>
