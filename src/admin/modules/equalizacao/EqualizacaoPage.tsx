...
const makeKartForm = (kart?: Kart | null): KartFormState => ({
  ...
  modelo: kart?.modelo || 'Kart Aluguel',
  motor: kart?.motor || '',
  ...
  const filteredKarts = useMemo(() => {
    ...
    return karts
      .filter((kart) => {
        ...
        const state = equalizationState(kart.media_equalizacao_ms, kart.numero);
        const haystack = [kart.numero, kart.chassi_numero, kart.sensor_numero, kart.modelo, kart.motor].filter(Boolean).join(' ').toLocaleLowerCase('pt-BR');
  ...
  const handleKartSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const payload = {
        ...
        modelo: kartForm.modelo.trim() || 'Kart Aluguel',
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[980px] w-full text-left">
            <thead className="border-b border-zinc-800">
              <tr>
                {['Kart / categoria', 'Chassi', 'Placa / sensor', 'Média da equalização', 'Manutenção', 'Situação', 'Ações'].map((label) => <th className="whitespace-nowrap px-4 py-3 text-[10px] font-bold uppercase tracking-[.1em] text-zinc-500" key={label}>{label}</th>)}
              </tr>
            </thead>
            <tbody>
              {loading ? Array.from({ length: 6 }, (_, index) => <tr className="border-b border-zinc-800/60" key={`loading-${index}`}><td className="px-4 py-4" colSpan={7}><span className="block h-5 animate-pulse rounded bg-zinc-800/70" /></td></tr>) : null}
              {!loading && filteredKarts.map((kart) => {
                ...
                return (
                  <tr className={`border-b border-zinc-800/60 transition-colors last:border-0 ${isSelected ? 'bg-brand-500/5' : 'hover:bg-zinc-800/30'}`} key={kart.id}>
                    ...
                    <td className="px-4 py-3.5 text-sm text-zinc-300"><span className="block">Placa {kart.numero}</span><span className={kart.sensor_numero ? 'text-zinc-500' : 'text-amber-300'}>Sensor {kart.sensor_numero || 'não cadastrado'}</span></td>
        ...
        <section className="rounded-xl border border-zinc-800 bg-zinc-900/60 shadow-card" id="ficha-kart">
          <header className="flex flex-col gap-4 border-b border-zinc-800 px-4 py-5 md:flex-row md:items-start md:justify-between md:px-5">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h2 className="text-lg font-semibold text-zinc-50">Kart {selectedKart.numero}</h2><KartCategoryBadge numero={selectedKart.numero} /><EqualizationStateBadge kart={selectedKart} /></div><p className="mt-1 text-sm text-zinc-400">Chassi {selectedKart.chassi_numero || 'não informado'} · Sensor {selectedKart.sensor_numero || 'não cadastrado'} · {selectedKart.modelo}</p></div>
            </div>
