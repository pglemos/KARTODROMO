...
import { fetchLapTimePassings, fetchLapTimeRacingCompetitors, fetchLapTimeRacingsPage } from '@/lib/livetime/laptime-racings';
...
async function handleLaptimePassings(url: URL, response: http.ServerResponse) {
  const sqlOptions = resolveLaptimeSqlOptions('racings');
  if (!sqlOptions) {
    sendJson(response, 503, { error: 'laptime_sql_not_configured' });
    return;
  ...
  if (!competitorId || !racingId) {
    sendJson(response, 400, { error: 'competitorId and racingId required' });
    return;
  }

  try {
    const rows = await fetchLapTimePassings(sqlOptions, competitorId, racingId);
    sendJson(response, 200, rows);
  } catch (error) {
    sendJson(response, 502, { error: error instanceof Error ? error.message : 'laptime_sql_query_failed' });
  }
...
const server = http.createServer((request, response) => {
  ...
  if (url.pathname === '/api/laptime-passings') {
    void handleLaptimePassings(url, response);
    return;
  }
