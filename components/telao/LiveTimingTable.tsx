...
function cardFieldStyle(field: TelaoField, value: string, layout: TelaoLayoutConfig): CSSProperties | undefined {
  if (field !== 'name' || !value) return undefined;

  return {
    '--fit-name-font-size': `${fitCardNameFontSize({
      text: value,
...
function tableStyle(layout: TelaoLayoutConfig): CSSProperties {
  return {
    ...
    '--layout-line-width': `${layout.lineWidth}px`,
          ...
          return (
            <div key={position} className={className}>
              {fields.map((field) => {
                ...
                return (
                  <span key={field} className={`card-field card-${field}`} style={cardFieldStyle(field, value, layout)} title={field === 'name' ? driver?.name || '' : undefined}>
                    {value}
                  </span>
                );
              })}
